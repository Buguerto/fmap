...
# Patron, los puntos deben moverse solo un lugar en una direccion a la vez.
#lista_patron = [1, 0, -1]
# Las seeds se generan invocando un numero aleatorio del patron cada vez que el codigo rota.
#seedx = (lista_patron[np.random.randint(0,3)])
#seedy = (lista_patron[np.random.randint(0,3)])
#seed = (seedx, seedy)
#print(seed)
# La coordenada inicial queda determinada una sola vez, es coordinate_f. A partir de esa coordenada se aplica el patron
#coordinate_f = (np.random.randint(1, 1000),np.random.randint(1, 1000))
#print(coordinate_f)
...
while npuntos < 20000:
    ...
    if npuntos == 300000:
        fmap.show()
        fmap = fmap.filter(ImageFilter.EDGE_ENHANCE)
        fmap.show()
        fmap = fmap.filter(ImageFilter.CONTOUR)
        fmap.show()
        fmap = fmap.filter(ImageFilter.CONTOUR)
        fmap.show()
        fmap = fmap.filter(ImageFilter.CONTOUR)
